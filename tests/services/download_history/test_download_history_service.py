from sqlalchemy.orm import Session

from app.services.company import CompanyService
from app.services.download_history import DownloadHistoryService
from app.services.report_file import ReportFileService
from app.services.sec.recent_report_metadata import RecentReportMetadata


def test_create_download_history_record(db_session: Session) -> None:
    company = CompanyService().create(
        db_session,
        name="Apple",
        cik="0000320193",
        ticker="AAPL",
    )
    report_file = ReportFileService().create(
        db_session,
        company_id=company.id,
        report_type="10-K",
        report_metadata=RecentReportMetadata(
            accession_number="0000320193-24-000001",
            primary_document="aapl-10k.htm",
            filing_date="2024-10-01",
            filing_url="https://www.sec.gov/Archives/edgar/data/320193/1/aapl-10k.htm",
        ),
        stored_file_name="apple_report.pdf",
        created_by=1,
    )

    record = DownloadHistoryService().create(
        db_session,
        report_file_id=report_file.id,
        stored_file_name="apple_report.pdf",
        downloaded_by=2,
    )

    assert record.id > 0
    assert record.report_file_id == report_file.id
    assert record.stored_file_name == "apple_report.pdf"
    assert record.downloaded_by == 2
